...
describe('ServiceUrlExtractor', () => {
  ...
  describe('getUrl', () => {
    ...
    it('should work with different URL formats', () => {
      ...
      expect(ServiceUrlExtractor.getUrl(urls, 'knowledge-base')).toBe('https://api.example.com:8080/v1/health');
      expect(ServiceUrlExtractor.getUrl(urls, 'auth-service')).toBe('http://192.168.1.1:3000/status');
    });
    ...
    it('should handle URLs with query parameters', () => {
      ...
      expect(ServiceUrlExtractor.getUrl(urls, 'knowledge-base')).toContain('?detailed=true');
      expect(ServiceUrlExtractor.getUrl(urls, 'auth-service')).toContain('?format=json');
    });
    ...
    it('should throw ValidationError for unknown service', () => {
      ...
      expect(() => ServiceUrlExtractor.getUrl(mockUrls, 'unknown-service')).toThrow(ValidationError);
    });
  });
});
